...
/* page permission for read only */
#define PERMISSION_READ  0x1

/* page permission for write */
#define PERMISSION_WRITE  0x2

/* page permission for execute */
#define PERMISSION_EXECUTE  0x4

/* page permission for read and execute */
#define PERMISSION_READ_EXECUTE (PERMISSION_READ | PERMISSION_EXECUTE)

/* page permission for read/write/execute*/
...
typedef union {
	struct {
		...
		uint64_t rip;
		uint64_t gva; /* GVA for the event and only valid for memory events */
	} data;

	struct {
		uint64_t head; /* next sequence number */
		uint64_t test;
	} meta;
} log_entry_t;
...
#define LOGS_PER_CPU (ENTRIES_PER_CPU - 1)
...
static inline uint64_t get_last_seq_num(log_entry_t cpu_log_buffer_start[])
{
	log_entry_t *meta_entry;

	meta_entry = &cpu_log_buffer_start[0];

	return meta_entry->meta.head;
}
